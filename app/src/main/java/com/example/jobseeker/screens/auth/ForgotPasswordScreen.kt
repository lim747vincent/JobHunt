...
/**
 * Created 26-02-2024 at 05:09 pm
 */

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun ForgotPasswordScreen(navController: NavController) {
    var jobseeker_email by remember { mutableStateOf("") }
...
//        )
            ...
            OutlinedTextField(
                value = jobseeker_email,
                onValueChange = { jobseeker_email = it },
                label = { Text("Email") },
                modifier = Modifier
                    .fillMaxWidth()
                    .padding(8.dp),
            ...
            Button(
                onClick = {
                    if (jobseeker_email.isNotBlank() && !isLoading) {
                        isLoading = true
                        when {
                            // Check if email is empty
                            jobseeker_email.isEmpty() -> {
                                Toast.makeText(context, "Email cannot be empty", Toast.LENGTH_SHORT).show()
                                isLoading = false
                            }
                            // Validate email format
                            !isValidEmail(jobseeker_email) -> {
                                Toast.makeText(context, "Please enter a valid email address", Toast.LENGTH_SHORT).show()
                                isLoading = false
                            }
                            else -> {
                                resetPassword(auth, jobseeker_email, context) { success, errorMessage ->
                                    isLoading = false
                                    if (success) {
                                        Toast.makeText(
                                            context,
...
fun resetPassword(auth: FirebaseAuth, email: String, context: Context, callback: (Boolean, String?) -> Unit) {

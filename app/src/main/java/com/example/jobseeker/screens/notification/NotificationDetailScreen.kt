...
@Composable
fun NotificationDetailScreen(navController: NavController, jobseekerViewModel: JobseekerViewModel, windowSize: WindowSize) {
...
//                            text = "Notification Details Screen",
                            text = notificationUiState.jobseeker_notification_title,
        ...
        Column(
            ...
            if(notificationUiState != null){
                ...
                when(windowSize.height){
                    WindowType.Medium->{
                        Text(
                            text = "Title: " + notificationUiState.jobseeker_notification_title,
                            fontSize = 32.sp,
                            lineHeight = 30.sp,
                        ...
                        Text(
                            text = "Detail text: " + notificationUiState.jobseeker_notification_detail_text,
                            fontSize = 20.sp,
                            color = colorResource(id = R.color.light_purple)
                        )
                        Spacer(modifier = Modifier.height(30.dp))
                        Text(
                            text = "Date: " + notificationUiState.jobseeker_notification_date,
                            fontSize = 16.sp,
                            color = colorResource(id = R.color.light_purple)
                        )
                        Text(
                            text = "Sender: " + notificationUiState.jobseeker_notification_sender,
                            fontSize = 16.sp,
                            color = colorResource(id = R.color.light_purple)
                        )
                        Text(

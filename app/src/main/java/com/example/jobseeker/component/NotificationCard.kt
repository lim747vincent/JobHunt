...
@Composable
fun NotificationCard(notification: JobseekerNotificationUiState, modifier: Modifier = Modifier, jobseekerViewModel: JobseekerViewModel,
                     onClick:()->Unit) {
    var isRead = remember { mutableStateOf(notification.jobseeker_notification_isRead) }

    val onClickNotification = {
        if(isRead.value == false){
            isRead.value = true
            jobseekerViewModel.updateJobseekerNotificationStatus(notification)
        }
    }
    ...
    Card(colors = CardDefaults.cardColors(
        ...
        Column(modifier= Modifier
            ...
            Row(modifier = Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.Start){
                Text(text = notification.jobseeker_notification_title,fontSize=25.sp, color= if(isRead.value!=true){
            ...
            Row(modifier = Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.Start){
                Text(text = notification.jobseeker_notification_brief_text,color = if(isRead.value!=true){
                    colorResource(id = R.color.light_purple)
                }else{
                    colorResource(id = R.color.grey)
                }, fontWeight = if (isRead.value != true){
            ...
            Row(modifier = Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.SpaceBetween){
                Text(text = notification.jobseeker_notification_sender,color = if(isRead.value!=true){
                    colorResource(id = R.color.light_purple)
                }else{
                    colorResource(id = R.color.grey)
                }, fontWeight = if (isRead.value != true){
                ...
                Text(text = notification.jobseeker_notification_date,color = if(isRead.value!=true){
                    colorResource(id = R.color.light_purple)
                }else{
                    colorResource(id = R.color.grey)
                }, fontWeight = if (isRead.value != true){
